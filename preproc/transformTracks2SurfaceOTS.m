...
for ss = [1]
    ...
    track_dir =fullfile('/sni-storage/kalanit/biac2/kgs','projects', 'PredictFuncFromStruct', 'data4Predict', strcat('Subject_',num2str(ss)),'Volume');
    ...
    for hem=1:2
        ...
        for p=1:length(proj_values)
            ...
            unix(['mri_vol2surf --mov ' map_path ' ' ...
                '--reg ' fullfile(surf_dir,'register.dat') ' --hemi lh --interp trilin --o ' ...
                fullfile(surf_out_dir,strcat(map_name, '_lh_proj_', num2str(proj_values(p)), '.mgh'))...
                ' --projfrac ' num2str(proj_values(p))]); % left hemi
            else
            unix(['mri_vol2surf --mov ' map_path ' ' ...
                '--reg ' fullfile(surf_dir,'register.dat') ' --hemi rh --interp trilin --o ' ...
                fullfile(surf_out_dir,strcat(map_name, '_rh_proj_', num2str(proj_values(p)), '.mgh'))...
                ' --projfrac ' num2str(proj_values(p))]); % left hemi
            end
        end
        ...
        unix(['mri_concat --i ' strcat(map_name, '_lh_proj_*')...
            ' --o ' fullfile(surf_out_dir,strcat(map_name, '_lh_proj_max.mgh'))...
            ' --max']);
        else
        ...
        unix(['mri_concat --i ' strcat(map_name, '_rh_proj_*')...
            ' --o ' fullfile(surf_out_dir,strcat(map_name, '_rh_proj_max.mgh'))...
            ' --max']);
        end
            ...
            %             map_name '_lh_proj05.mgh --trgsubject fsaverage-bkup --trgsurfval ' ...
            %             map_stem '_lh_proj05_regFrom_subj_' subject '.mgh --hemi lh']); % left hemi
            %         unix(['mri_surf2surf --srcsubject ' fs_id ' --srcsurfval ' ...
            %             map_name '_rh_proj05.mgh --trgsubject fsaverage-bkup --trgsurfval ' ...
            %             map_stem '_rh_proj05_regFrom_subj_' subject '.mgh --hemi rh']);
        end
    end
end
end
...
for s=11
    for r=1:length(ROIs)
        ...
        cd(fullfile('/share/kalanit/biac2/kgs/projects/PredictFuncFromStruct/PredictMatrixesMNI/'));
        load(strcat('Subject_',num2str(s),'_lh_OTS_fsavg_regression.mat'));
        label = read_label(fs_ids{s},strcat('aparc2009/predictFuncFromStructBoundaries/','lh_OTS_from_fsaverage_manual'));
        
        label(:,5)=predictorsA(:,1);
        ...
        cd(fullfile('/share/kalanit/biac2/kgs/projects/PredictFuncFromStruct/OutputMaps/RegressionGrp1'));
        save_mgh(vol,strcat('Actual_Subject_',num2str(s),'_',ROIs{r},'_regression_T1_Grp1_',predictors{:},'.mgh'), M, mr_params);

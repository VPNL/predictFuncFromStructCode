...
for s=[1 3 4 6 7 9 10 12 15 17 19 20 21 23 24 25 26 27 28 29] 
    for r=1:length(ROIs)
        ...
        cd(fullfile('/share/kalanit/biac2/kgs/projects/PredictFuncFromStruct/OutputMatrixes/Leave1OutLinearGrp2'));
        load(strcat('Subject_',num2str(s),'_Grp2_',ROIs{r},'_regression_',predictors{:},'.mat'));
        label = read_label(fs_ids{s},strcat('aparc2009/predictFuncFromStructBoundaries/','lh_OTS_from_fsaverage_manual'));
        
        label(:,5)=readResponsePredicted(:,1);
        ...
        save_mgh(vol,strcat('Predicted_Subject_',num2str(s),'_',ROIs{r},'_regression_linModel_Grp2_',predictors{:},'.mgh'), M, mr_params);
